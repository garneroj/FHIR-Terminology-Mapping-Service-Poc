...
For this POC, two codes will suffice.
...
IMPORTANT: If you wish to change the database name, password, username, port, you must update this file. Hardcoded in this POC, to keep things simple.
But do not hardcode if you go beyond a POC.  
...
This will bring the latest docker image for *postgres*.

execute
[source,script]
-----------------
$ sudo docker run -d --name terminologyContainer -p 5432:5432 -e POSTGRES_PASSWORD=Pass123 -e POSTGRES_USER=terminologypostgresuser postgres
-----------------
...
* image port *5432* will be *5432* too, in your box. (_-p_)
...
We need to config the legacy codes, LOINC codes, and its mapping
...
To do that we will create such code systems and such concept map resources.
...
Please run the script. It will skip the load if resource is already created. 

You should read something like:

[source,script]
-----------------
$ ./term-server-config/term-server-config.sh 
==========================================
